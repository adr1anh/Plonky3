...
impl<F: TwoAdicField> TwoAdicSubgroupDft<F> for NaiveDft {
    ...
    fn dft_batch(&self, coefficients: RowMajorMatrix<F>) -> RowMajorMatrix<F> {
        ...
        let mut evaluations = RowMajorMatrix::new(vec![F::ZERO; w * h], w);
        for (eval_r, point) in g.powers().take(h).enumerate() {
            for (coeff_r, point_power) in point.powers().take(h).enumerate() {
                for c in 0..w {
                    evaluations.values[eval_r * w + c] +=
                        point_power * coefficients.values[coeff_r * w + c];
                }
            }
        }
